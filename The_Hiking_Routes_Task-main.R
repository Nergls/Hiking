...
setwd("C:/Users/NargizSafaraliyeva/OneDrive - geo.uzh.ch/Desktop/Applications/Portfolio/Hiking routes")
pati_main <- "./"
...
for (g in seq(nrow(gpx_file))) { #g=1 #Debug when is needed
  ...
  # elevation_profile <- ggplot(coords_df, aes(x = cumulative_distance / 1000, y = elevation)) +
  #   geom_line(color = "cyan") +
  #   geom_density_ridges(fill = "cyan", rel_min_height = 0.01, aes(y=elevation, group = elevation))+
  ...
  # ggplot(coords_df, aes(x = cumulative_distance / 1000, y = id_name, fill = 0.5 - abs(0.5 - stat(ecdf)))) +
  #   stat_density_ridges(geom = "density_ridges_gradient", calc_ecdf = TRUE) +
  #   scale_fill_viridis_c(name = "Tail probability", direction = -1)
  
  # Create the elevation profile plot with gradient fill
  elevation_profile <- ggplot(coords_df, aes(x = cumulative_distance / 1000, y = elevation, fill = elevation,alpha = 0.7)) +
    geom_area() +  # Adding the area under the line with gradient fill
    geom_line(color = "purple", linewidth =1 ) +
    ylim(min(coords_df$elevation, na.rm = TRUE),max(coords_df$elevation, na.rm = TRUE))+
...
all_elevation$id_name <- factor(unique(all_elevation$id_name))
...
elevation_prof <- ggplot(all_elevation, aes(x = (cumulative_distance / 1000), y = elevation, fill = id_name)) +
  #geom_line(color = "blue", group = all_elevation$id_name) +
  geom_area(alpha=0.6 , size=.5, colour="white") +
  scale_fill_viridis(discrete = T) +
  labs(title = paste0("Elevation Profile of ",all_elevation$id_name, " hike"),
       #subtitle = height_diff_text,  # Adding the subtitle
       x = "Distance (km)", y = "Elevation (m)") +
  theme_minimal()
elevation_prof

ggplot(all_elevation, aes(x = cumulative_distance / 1000, y = id_name, fill = elevation)) +
  stat_density_ridges(geom = "density_ridges_gradient", calc_ecdf = TRUE) +
  scale_fill_viridis_c(name = "Tail probability", direction = -1)
