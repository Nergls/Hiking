...
ggsave(filename = "./hiking_routes_switzerland.png", 
       plot = ggsf, dpi = 300,  width = 10,  height = 8, 
       units = "in")
graphics.off()
#print(ggsf)
#ggplotly(ggsf)
...
elevation_proffi <- ggplot(all_elevation, aes(x = cumulative_distance / 1000, y = elevation, group = id_name, fill = id_name)) +
  ...
  coord_cartesian(ylim = c(min(all_elevation$elevation, na.rm = TRUE), NA))
#elevation_proffi
ggsave(filename = "./Elevation Profile of all hikes.png", 
       plot = elevation_proffi, dpi = 300,  width = 20,  height = 18, 
       units = "in")
# Set y-axis limit starting from the minimum elevation #xlim = c(0,10)
...
elevation_profffi <- ggplot(all_elevation, aes(x = cumulative_distance / 1000, y = elevation, group = id_name, fill = id_name)) +
  ...
  facet_grid(rows = vars(id_name), scales = 'free') +  # Stack vertically with free y-axis scaling
  coord_cartesian(ylim = c(min(all_elevation$elevation, na.rm = TRUE), NA))  # Set y-axis limit starting from the minimum elevation
ggsave(filename = "./Elevation Profile of all hikes-v2.png", 
       plot = elevation_profffi, dpi = 300,  width = 10,  height = 8, 
       units = "in")
# Display the plot
...
elevation_proffi_combined <- ggplot(all_elevation, aes(x = cumulative_distance / 1000, y = elevation, color = id_name)) +
  ...
  theme(legend.position = 'right', 
        ...
        strip.text.x = element_text(size = rel(0.9))) +
  coord_cartesian(ylim = c(min(all_elevation$elevation, na.rm = TRUE), NA)) 
ggsave(filename = "./Elevation Profile of all hikes-v3.png", 
       plot = elevation_proffi_combined, dpi = 300,  width = 15,  height = 13, 
       units = "in")
# Set y-axis limit starting from the minimum elevation
# Display the combined plot
